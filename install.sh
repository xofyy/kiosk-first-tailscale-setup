...
# -----------------------------------------------------------------------------
# Ethernet Interface Detection (Onboard vs PCIe)
# -----------------------------------------------------------------------------

# Known motherboard manufacturers - subsystem_vendor IDs
declare -A KNOWN_MB_VENDORS=(
    ["0x1462"]="MSI"
    ["0x1043"]="ASUS"
    ["0x1458"]="Gigabyte"
    ["0x1849"]="ASRock"
...
get_onboard_interface() {
    # Find onboard (motherboard) ethernet interface using subsystem_vendor
    for iface in /sys/class/net/*/device/subsystem_vendor; do
        [[ -e "$iface" ]] || continue
        local ifname=$(echo "$iface" | cut -d/ -f5)

        # Skip virtual and wireless interfaces
        [[ "$ifname" =~ ^(lo|docker|veth|br-|tailscale|virbr) ]] && continue
        [[ -d "/sys/class/net/$ifname/wireless" ]] && continue

        local vendor=$(cat "$iface" 2>/dev/null)
        if [[ -n "${KNOWN_MB_VENDORS[$vendor]}" ]]; then
            echo "$ifname"
            return
        fi
    done
...
get_pcie_interface() {
    # Find PCIe slot ethernet interface (non-onboard)
    local onboard="$1"
    ...
    for iface in /sys/class/net/*/device/subsystem_vendor; do
        ...
        # Skip the onboard interface
        [[ "$ifname" == "$onboard" ]] && continue

        local vendor=$(cat "$iface" 2>/dev/null)
        if [[ -z "${KNOWN_MB_VENDORS[$vendor]}" ]]; then
            echo "$ifname"
            return
        fi
    done
...
# Detect interfaces
ONBOARD_IFACE=$(get_onboard_interface)
PCIE_IFACE=$(get_pcie_interface "$ONBOARD_IFACE")

# Fallback: if no onboard detected, use default route interface
if [[ -z "$ONBOARD_IFACE" ]]; then
    ONBOARD_IFACE=$(ip route | grep default | awk '{print $5}' | head -1)
    [[ -z "$ONBOARD_IFACE" ]] && ONBOARD_IFACE="eth0"
    log "No onboard interface detected, using default: $ONBOARD_IFACE"
else
    log "Onboard interface detected: $ONBOARD_IFACE"
fi

if [[ -n "$PCIE_IFACE" ]]; then
    log "PCIe interface detected: $PCIE_IFACE"
else
    log "No PCIe interface detected"
fi
...
# Generate netplan config based on detected interfaces
if [[ -n "$PCIE_IFACE" ]]; then
    # Both onboard and PCIe interfaces
...
# Onboard: $ONBOARD_IFACE, PCIe: $PCIE_IFACE
network:
  version: 2
    ...
    $PCIE_IFACE:
      ...
      nameservers:
        addresses:
          ...
          - 8.8.4.4
EOF
    log "Netplan configured for both interfaces: $ONBOARD_IFACE (onboard), $PCIE_IFACE (pcie)"
else
    # Only onboard interface
    cat > /etc/netplan/01-network-manager.yaml << EOF
# Network configuration managed by NetworkManager
# Generated by ACO Maintenance Panel installer
# Onboard: $ONBOARD_IFACE
network:
  version: 2
  renderer: NetworkManager
  ethernets:
    $ONBOARD_IFACE:
      ...
      nameservers:
        addresses:
          ...
          - 8.8.4.4
EOF
    log "Netplan configured for single interface: $ONBOARD_IFACE"
fi
